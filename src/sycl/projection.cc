...
void spherical_projection_sycl(sycl::queue& Q, IsomerBatch<T,K>& batch, const LaunchPolicy policy){
    using namespace spp;
...
template void spherical_projection_sycl<float, uint16_t>(sycl::queue& Q, IsomerBatch<float,uint16_t>& batch, const LaunchPolicy policy);