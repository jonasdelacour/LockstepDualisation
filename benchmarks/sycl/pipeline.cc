...
int main(int argc, char** argv) {
	...
	buckygen_time += ns_time(before_time-after_time).count(); // Time spent waiting for buckygen to generate g
    ...
    vector<double> times_memcpy  (N_runs,0); //Times in nanoseconds.
...
#define time_operation(kernel,name,t0,t1)					    \
	...
	auto bucky_time = fill(batches[j]);
	IFW(i) times_generate[i- N_warmup] += bucky_time;
      } 
      sync_and_time(T0);
      time_operation(nop_kernel,        times_memcpy,T0,T1);
      time_operation(dualise_sycl_v1<6>,times_dual,  T1,T2);	
    ...
    std::cout << "Generate: " << mean(times_generate)/batch_size << ", " << stddev(times_generate)/batch_size << " ns \n";
    std::cout << "Memcpy: " << mean(times_memcpy)/batch_size << ", " << stddev(times_memcpy)/batch_size << " ns \n";
    ...
    file << N << ","
         ...
         << mean(times_memcpy)/batch_size << ","
         << stddev(times_memcpy)/batch_size << ","
