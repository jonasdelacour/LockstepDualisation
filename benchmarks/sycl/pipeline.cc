...
int main(int argc, char** argv) {
    ...
    Graph G(N);
    auto fill = [&](IsomerBatch<float,uint16_t>& batch)
    {
    ...
    for(int i = 0; i < N_runs + N_warmup; i++){
        ...
        for(int j = 0; j < N_gpus; j++) fill(batches[j]);
        for(int j = 0; j < N_gpus; j++) {Qs[j].wait();} 
        auto T0 = std::chrono::steady_clock::now(); IFW(i) times_generate[i - N_warmup] = std::chrono::duration<double, std::nano>(T0 - start).count();
