...
int main(int argc, char** argv) {
    ...
    auto fill = [&](IsomerBatch<float,uint16_t>& batch) -> double 
    {
    ...
    size_t ii = 0;
        while( BuckyGen::next_fullerene(BuckyQ,g) && (isomer_ix < M_graphs) && (ii<batch.m_capacity)){
            ...
            buckygen_time += duration<double,nanoseconds::period>(before_time-after_time).count(); // Time spent waiting for buckygen to generate g
    ...
    for(int i = 0; i < N_runs + N_warmup; i++){
        ...
        IFW(i) {for(int j = 0; j < N_gpus; j++) times_generate[i- N_warmup] = fill(batches[j]);} else {for(int j = 0; j < N_gpus; j++) fill(batches[j]);}
        for(int j = 0; j < N_gpus; j++) {Qs[j].wait();} 
        auto T0 = std::chrono::steady_clock::now();
        for(int j = 0; j < N_gpus; j++) nop_kernel(Qs[j], batches[j], LaunchPolicy::ASYNC);
        for(int j = 0; j < N_gpus; j++) {Qs[j].wait();} 
        auto T1 = std::chrono::steady_clock::now(); IFW(i) times_memcpy[i - N_warmup] = std::chrono::duration<double, std::nano>(T1 - T0).count();
        for(int j = 0; j < N_gpus; j++) dualise_sycl_v1<6>(Qs[j], batches[j], LaunchPolicy::ASYNC);
        for(int j = 0; j < N_gpus; j++) {Qs[j].wait();} 
        auto T2 = std::chrono::steady_clock::now(); IFW(i) times_dual[i - N_warmup] = std::chrono::duration<double, std::nano>(T2 - T1).count();
        for(int j = 0; j < N_gpus; j++) tutte_layout_sycl(Qs[j], batches[j], LaunchPolicy::ASYNC);
        for(int j = 0; j < N_gpus; j++) {Qs[j].wait();} 
        auto T3 = std::chrono::steady_clock::now(); IFW(i) times_tutte[i - N_warmup] = std::chrono::duration<double, std::nano>(T3 - T2).count();
        for(int j = 0; j < N_gpus; j++) spherical_projection_sycl(Qs[j], batches[j], LaunchPolicy::ASYNC);
        for(int j = 0; j < N_gpus; j++) {Qs[j].wait();} 
        auto T4 = std::chrono::steady_clock::now(); IFW(i) times_project[i - N_warmup] = std::chrono::duration<double, std::nano>(T4 - T3).count();
        for(int j = 0; j < N_gpus; j++) forcefield_optimise_sycl(Qs[j], batches[j], 5*N, 5*N, LaunchPolicy::ASYNC);
        for(int j = 0; j < N_gpus; j++) {Qs[j].wait();} 
        auto T5 = std::chrono::steady_clock::now(); IFW(i) times_opt[i - N_warmup] = std::chrono::duration<double, std::nano>(T5 - T4).count();
    }
    ...
    remove_outliers(times_memcpy); //Removes data points that are more than 2 standard deviations from the mean. If there are less than 3 data points, this does nothing.
    ...
    std::cout << "Generate: " << mean(times_generate)/N_graphs << ", " << stddev(times_generate)/N_graphs << " ns \n";
    std::cout << "Memcpy: " << mean(times_memcpy)/N_graphs << ", " << stddev(times_memcpy)/N_graphs << " ns \n";
